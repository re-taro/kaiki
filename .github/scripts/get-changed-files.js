...
const githubApi = (path) => {
  return new Promise((resolve, reject) => {
    const options = {
      ...
      path,
      headers: {
        'User-Agent': 'kaiki-changed-files',
...
async function getPrChangedFiles(repository, prNumber) {
  ...
  let prFiles;
  do {
    // Sequential pagination requires await in loop
    prFiles = await githubApi(`/repos/${repository}/pulls/${prNumber}/files?per_page=${perPage}&page=${page}`);
