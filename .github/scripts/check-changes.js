 ...
 *   node check-changes.js --packages kaiki_napi --paths napi/
 *
 * Exclude mode: skips only if ALL changed files belong to excluded crate directories.
 *   node check-changes.js --exclude kaiki_notify --paths napi/,npm/
 *
 * Paths-only mode: run if any changed file matches the given paths (no cargo tree).
 *   node check-changes.js --paths wasm/,npm/kaiki/
 */

import { getChangedFiles } from './get-changed-files.js';
import { getCrateDependencies, checkFilesAffectCrates } from './utils.js';
 ...
 * @returns {boolean}
 */
const shouldRunInclude = (changedFiles, packages, paths) => {
 ...
 * @returns {boolean}
 */
const shouldRunExclude = (changedFiles, excludeCrates, paths) => {
  if (changedFiles === null) {
    console.error('No changed files list (manual trigger or error) - will run');
    return true;
  }
 ...
 */
const shouldRunPathsOnly = (changedFiles, paths) => {
  if (changedFiles === null) {
    console.error('No changed files list (manual trigger or error) - will run');
    return true;
  }
